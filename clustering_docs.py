...
class ClusteringDocs(object):
    ...
    def runOtimizaPesoAtributo(self, weight=None):
        ...
        for column in features_cluster.columns[1:]: #iterate over all columns but the first one, in my case, just one the cluster so it's fine
            list_features_cluster.append(features_cluster[column])
        
        for clusters in range(2, math.floor(np.sqrt(self.tav.shape[0]))):
        #for clusters in range(2, 3):
            weight_array_features = np.empty_like(features_cluster[["feature"]])
