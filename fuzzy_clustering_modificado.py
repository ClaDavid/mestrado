...
class FCMMOD:
    ...
    def distance_squared_normal(self, x, c):
        """
    ...
    def distance_squared(self, x, c, weight=None):
        """
        Compute the Euclidean distance
        :param x: is a single point from the original data X
        :param c: is a single point that represent a center or a cluster
        ...
        """
        if weight is None:
            weight = np.repeat(1, len(x), axis=0)
        weight = weight/sum(weight)
        sum_of_sq = 0.0
        for i in range(len(x)):
            sum_of_sq += ((x[i]-c[i]) ** 2)*weight
        return sum_of_sq

    def compute_membership_single(self, X, datapoint_idx, cluster_idx, weight=None):
        """
        :param datapoint_idx:
        :param cluster_idx:
        ...
        clean_X = X
        d1 = self.distance_squared(clean_X[datapoint_idx], self.cluster_centers_[cluster_idx], weight)
        sum1 = 0.0
        for c in self.cluster_centers_:  # this is to compute the sigma
            d2 = self.distance_squared(c, clean_X[datapoint_idx], weight)
            if np.any(d2 == 0.0):
                d2 = SMALL_VALUE
    ...
    def update_membership(self, X, weight=None):
        """
        ...
        for i in range(X.shape[0]):
            for c in range(len(self.cluster_centers_)):
                self.u[i][c] = self.compute_membership_single(X, i, c, weight[i])

    def fit(self, X, y=None, hard=True, weight=None):
        """
        :param X:
        ...
        for i in range(self.max_iter):
            ...
            self.update_membership(X, weight)
            membership_history.append(self.u.copy())
